...
interface MatchFormProps {
  initialData?: Match
  players: Array<Profile | (SeasonPlayer & { type: "temp" })>
  currentUserId: string
  activeSeasonName?: string | null
}

export function MatchForm({ initialData, players, currentUserId, activeSeasonName }: MatchFormProps) {
  const [player1, setPlayer1] = useState(initialData?.player1_id || (initialData?.temp_player1_id ? `temp:${initialData.temp_player1_id}` : ""))
  const [player2, setPlayer2] = useState(initialData?.player2_id || (initialData?.temp_player2_id ? `temp:${initialData.temp_player2_id}` : ""))
  const [player3, setPlayer3] = useState(initialData?.player3_id || (initialData?.temp_player3_id ? `temp:${initialData.temp_player3_id}` : ""))
  const [player4, setPlayer4] = useState(initialData?.player4_id || (initialData?.temp_player4_id ? `temp:${initialData.temp_player4_id}` : ""))
  const [winnerTeam, setWinnerTeam] = useState<"1" | "2">(initialData?.winner_team?.toString() as "1" | "2" || "1")
  const [team1Score, setTeam1Score] = useState(initialData?.team1_score?.toString() || "0")
  const [team2Score, setTeam2Score] = useState(initialData?.team2_score?.toString() || "0")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Error al guardar la partida")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        {isLoading ? "Guardando..." : initialData ? "Actualizar partida" : "Registrar partida"}
      </Button>
